...
export default function KeysPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (formData.issueTime && formData.returnTime && formData.returnTime <= formData.issueTime) {
        ...
        <Card className="bg-[#251dc9] text-white">
          ...
          <CardContent>
            <form className="grid grid-cols-2 gap-4" onSubmit={handleSubmit}>
              ...
              {/* Person Name */}
              <Input name="personName" placeholder="Person Name" value={formData.personName} onChange={handleChange} required />

              {/* Key Type */}
              <Select onValueChange={(value) => setFormData({ ...formData, keyType: value })} required>
                <SelectTrigger>
              ...
              {/* Issue & Return Time */}
              <Input type="time" name="issueTime" value={formData.issueTime} onChange={handleChange} required />
